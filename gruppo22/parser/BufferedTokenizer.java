...
public class BufferedTokenizer implements Tokenizer {
	...
	private static enum Group {
		WHOLE_REG_EXP, SYMBOL, KEYWORD, SKIP, IDENT, NUM, // WHOLE_REG_EXP unused, needed only for assigning the correct
																											// value to the other constants
	}
	...
	static {
		/*
		 * definition of the regular expressions of all valid lexemes remark: groups
		 * must correspond to the ordinal of the corresponding enum Group constant
		 */

		final var regExUnion = "|";
		/*
		 * builds the regular expression for symbols symbols must be in reversed String
		 * order because the regular expression operator '|' is left-preferential! For
		 * instance '==' must come before '='
		 */
		var symbolSets = new TreeSet<>(Comparator.<String>naturalOrder().reversed()); // symbols used in the
																																									// regularexpression
